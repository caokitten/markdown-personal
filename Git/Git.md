...
- 目标文件位置执行,初始化目录为一个git仓库:
  ...
  ```shell
  git init 
  # git的初始化本质上是在该目录下创建一个.git文件以记录git信息,不会进行任何文件的删除与修改
  ```
...
主分支,用于存放生产环境可运行的稳定代码,最终发布版本的代码就在这里

#### develop

开发分支,日常开发的集成分支,所有功能开发完成后合并到这里,用于测试

#### feature/xxx

功能分支,开发新功能,完成后合并到develop分支并删除

#### fix/xxx

bug修复分支,修复develop分支的bug,完成后合并到develop分支并删除

#### hotfix/xxx

紧急修复分支,修复main分支上的紧急bug,完成后合并到main和develop分支并删除

### 切换分支前先提交本地的修改

修改提交时,其提交的分支为当前分支,若在不提交的情况下切换至其他分支,则此时修改会被定向为切换后的分支的修改
